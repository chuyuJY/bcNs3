...
int main(int argc, char *argv[]) {
    ...
    gridScenario.SetRows(gNbNum / 2); //基站分布为几行几列
    ...
    gridScenario.SetScenarioLength(20); // be distribuited.  UE分布在基站周围的范围大小
    // gridScenario.SetStartingPosition(Vector (-50, -20, 0));
    gridScenario.SetStartingPosition(Vector(-60, -20, 0));
    ...
    NetworkHelper networkHelper(ueNodes.GetN());

    int ueNum = ueNodes.GetN();
    for (int i = 0; i < ueNum; i++) {
        for (int j = 0; j < ueNum; j++) {
            if (i != j) {
                Ipv4Address ueAddress = ueIpIface.GetAddress(j);
                networkHelper.nodesConnectionsIps[i].push_back(ueAddress);
            }
        }
    }
    ApplicationContainer nodeApp = networkHelper.Install(ueNodes);
