...
int main(int argc, char *argv[]) {
    ...
    Client.SetAttribute("RemotePort", UintegerValue(dlPort));
    ...
    if (!disableDl) {
        ...
        for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
            ...
            Client.SetAttribute("RemoteAddress", AddressValue(ueAddress));
            clientApps.Add(Client.Install(remoteHost0));
        }
    }

    if (!disableUl) {
        Ipv4Address remoteHostAddr1 = internetIpIfaces1.GetAddress(1);
        Client.SetAttribute("RemotePort", UintegerValue(ulPort));
        Client.SetAttribute("RemoteAddress", AddressValue(remoteHostAddr1));
        serverApps.Add(ulPacketSinkHelper.Install(remoteHost1));
        for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
