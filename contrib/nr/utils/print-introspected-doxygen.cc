  ...
  for (const auto & tid : types)
      ...
      if (tid.MustHideFromDocumentation ())
	{
      ...
      for (uint32_t i = 0; i < TypeId::GetRegisteredN (); i++)
        {
          TypeId tmp = TypeId::GetRegistered (i);
          if (tmp.GetName() == tid.GetName())
            {
              nameMap[name] = i;
            }
        }
...
PrintConfigPaths (std::ostream & os, const TypeId tid)
{
  ...
  if (paths.empty ())
    ...
    {
      os << headingStart
         <<   "config_" << tid.GetUid () << " Config Paths"
	 << headingStop
	 << std::endl;
      os << std::endl;
      os << tid.GetName ()
...
PrintAttributes (std::ostream & os, const TypeId tid)
{
  ...
  while (tmp.GetParent () != tmp)
	...
	{
	  os << headingStart
	     <<   "attr_parent_" << tid.GetUid () << " Attributes defined in parent class "
	     <<   tmp.GetName ()
	     << headingStop
	     << std::endl;
...
PrintTraceSources (std::ostream & os, const TypeId tid)
{
  ...
  if (tid.GetTraceSourceN () == 0)
    ...
    {
      os << headingStart
         <<   "tracesources_" << tid.GetUid () << " TraceSources"
  ...
  while (tmp.GetParent () != tmp)
	...
	{
	  os << headingStart
	     << "tracesources_parent_" << tid.GetUid () << " TraceSources defined in parent class "
	     << tmp.GetName ()
	     << headingStop << std::endl;
